    ...
    editMethod: function editMethod(doc){ //extends sparql.editable,
        ...
        */
        ...
        if (request !== undefined) {
            ...
            if (!response) return null; // throw "No recorded HTTP GET response for document: "+doc; //could be still out
            return kb.any(response, tabulator.ns.httph("ms-author-via"));
        } else {
        ...
        _mutate_statement: function mutate_statement(st, callback, mode, newObject){
            ...
            switch (protocol){
                case 'SPARQL': //cool!
                    ...
                    break;
                case 'DAV': //I don't like this at all, updating the whole document doesn't make any sense. - Kenny
                case 'LOCALFILE':
                    try{
                        var documentString;
                        var request = kb.any(doc, tabulator.ns.link("request"));
