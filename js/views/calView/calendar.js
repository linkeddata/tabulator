...
YAHOO.widget.Calendar_Core.prototype.renderCellDefault = function(workingDate, cell) {
	cell.innerHTML = ""; //empties cell
	var link = document.createElement("a");
	//link.href=":void(null);";
	var yr = workingDate.getFullYear();
	var mth = workingDate.getMonth()+1;
	var day = workingDate.getDate();
	link.name=this.id+"__"+yr+"_"+mth+"_"+day;
	//link.href = ":void(null);";
	//link.appendChild(document.createTextNode(this.buildDayLabel(workingDate)));
	link.innerHTML = this.buildDayLabel(workingDate);
	cell.appendChild(link);

	// eventCell for displaying number of events or Events themselves
	var eventCell = cell.appendChild(document.createElement('a'));
};
...
function indexOfDateInArray(find, array) {
	// var array = this.selectedDates;
...
YAHOO.widget.Calendar_Core.prototype._indexOfSelectedFieldArray = function(find) {
	return indexOfDateInArray(find, this.selectedDates);
};
...
YAHOO.widget.Calendar.prototype.renderHeader = function() {
	...
	//linkLeft.href = ":" + this.id + ".previousMonth()";
	linkLeft.onclick = this.previousMonth;
	...
	//linkRight.href = ":" + this.id + ".nextMonth()";
	linkRight.onclick = this.nextMonth;
...
YAHOO.widget.CalendarGroup.prototype.callChildFunction = function(fnName, args) {
	for (var p=0;p<this.pageCount;++p) {
		...
		if (page[fnName]) {
			var fn = page[fnName];
			// .call allows you to assign a different 'this' object when calling an existing function.
			// function will take 'page' as context for evaluating.
			fn.call(page, args);
		}
	}	
}
