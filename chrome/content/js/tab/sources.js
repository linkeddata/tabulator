...
tabulator.SourceFetcher = function(store, timeout, async) {
    ...
    tabulator.SourceFetcher.XHTMLHandler.term = this.store.sym(this.thisURI
						                                       +".XHTMLHandler")
    tabulator.SourceFetcher.XHTMLHandler.toString=function () { return "XHTMLHandler" }
		            ...
		            if (dom.childNodes[c].nodeType == 1) {
			            ...
			            if (ns == tabulator.ns['rdf']) {
			                tabulator.log.info(xhr.uri + " seems to have a root element"
				                               ...
				                               + "it's RDF/XML.")
			                sf.switchHandler(tabulator.SourceFetcher.RDFXMLHandler,
					                         xhr, cb, [dom])
			                return
			            }
    ...
    tabulator.SourceFetcher.XMLHandler.term = this.store.sym(this.thisURI
						                                     +".XMLHandler")
    tabulator.SourceFetcher.XMLHandler.toString=function () { return "XMLHandler" }
    ...
    tabulator.SourceFetcher.HTMLHandler.term = this.store.sym(this.thisURI
						                                      +".HTMLHandler")
    tabulator.SourceFetcher.HTMLHandler.toString=function () { return "HTMLHandler" }
    ...
    tabulator.SourceFetcher.TextHandler.term = this.store.sym(this.thisURI
						                                      +".TextHandler")
    tabulator.SourceFetcher.TextHandler.toString = function () { return "TextHandler" }
    ...
    tabulator.rdf.Util.callbackify(this,['request', 'recv', 'load', 'fail', 'refresh',
			                             'retract', 'done'])
    ...
    */
    ...
    [tabulator.SourceFetcher.RDFXMLHandler,
     tabulator.SourceFetcher.XHTMLHandler,
     tabulator.SourceFetcher.XMLHandler,
     tabulator.SourceFetcher.HTMLHandler,
     tabulator.SourceFetcher.TextHandler,
     tabulator.SourceFetcher.N3Handler,
    ].map(this.addHandler)
...
//		               kb.sym(tabulator.rdf.Util.uri.docpart(rterm.uri)), this.appNode)
	    ...
	    if (typeof kb.anyStatementMatching(this.appNode,
					   tabulator.ns.link("protocol"),
					                       tabulator.rdf.Util.uri.protocol(uri))
	        == "undefined") {
